...
ConstantBuffer<Struct__GatherDOF_FloodFillFarCSCB> _GatherDOF_FloodFillFarCSCB : register(b0);
...
float2 ReadVec2STTextureRaw(in uint3 pxAndFrame, in Texture2DArray<float2> tex)
{
	...
	switch(_GatherDOF_FloodFillFarCSCB.LensRNGExtend)
	{
...
float2 ReadVec2STTexture(in uint3 pxAndFrame, in Texture2DArray<float2> tex)
{
	...
	if (_GatherDOF_FloodFillFarCSCB.JitterNoiseTextures)
...
{
	if (!_GatherDOF_FloodFillFarCSCB.GatherDOF_UseNoiseTextures)
	{
	...
	switch(_GatherDOF_FloodFillFarCSCB.LensRNGSource)
	{
...
#define FLOODFILL_TAP_COUNT _GatherDOF_FloodFillFarCSCB.GatherDOF_FloodFillTapCount
...
#define KernelSize _GatherDOF_FloodFillFarCSCB.GatherDOF_KernelSize
...
void csmain(uint3 DTid : SV_DispatchThreadID)
{
	...
	if (!_GatherDOF_FloodFillFarCSCB.GatherDOF_DoNearFieldFloodFill)
	{
	...
	// pass through if this is disabled
	if (!_GatherDOF_FloodFillFarCSCB.GatherDOF_DoFarFieldFloodFill)
	{
		BlurredFieldColorAlpha[px] = ColorCoc[px];
		return;
	}
	...
	uint noiseTextureFrameIndex = _GatherDOF_FloodFillFarCSCB.GatherDOF_AnimateNoiseTextures ? _GatherDOF_FloodFillFarCSCB.GatherDOF_FrameIndex : 0;
