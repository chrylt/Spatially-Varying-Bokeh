...
void s2h_drawArc(inout ContextGather ui, float2 pxCenter, float pxRadius, float4 color, float pxThickness, float startAngle, float endAngle);
// draw anti aliased circle
...
void s2h_drawArc(inout ContextGather ui, float2 pxCenter, float pxRadius, float4 color, float pxThickness, float startAngle, float endAngle)
{
	...
	float angleMask = (rel <= span) ? 1.0f : 0.0f;

	float halfT = pxThickness * 0.5f;
	float radialMask = saturate(halfT - abs(len - pxRadius));

	float mask = radialMask * angleMask;
	ui.dstColor = lerp(ui.dstColor, float4(color.rgb, 1), color.a * mask);
}
