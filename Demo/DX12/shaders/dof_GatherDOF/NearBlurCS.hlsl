...
float2 ReadVec2STTextureRaw(in uint3 pxAndFrame, in Texture2DArray<float2> tex)
{
	...
	switch(_GatherDOF_NearBlurCB.LensRNGExtend)
	{
...
float2 ReadVec2STTexture(in uint3 pxAndFrame, in Texture2DArray<float2> tex)
{
	...
	if ((bool)_GatherDOF_NearBlurCB.JitterNoiseTextures)
...
{
	if (!(bool)_GatherDOF_NearBlurCB.GatherDOF_UseNoiseTextures)
	{
	...
	switch(_GatherDOF_NearBlurCB.LensRNGSource)
	{
...
#define BLUR_TAP_COUNT _GatherDOF_NearBlurCB.GatherDOF_BlurTapCount
...
void csmain(uint3 DTid : SV_DispatchThreadID)
{
	...
	uint noiseTextureFrameIndex = (bool)_GatherDOF_NearBlurCB.GatherDOF_AnimateNoiseTextures ? _GatherDOF_NearBlurCB.GatherDOF_FrameIndex : 0;
