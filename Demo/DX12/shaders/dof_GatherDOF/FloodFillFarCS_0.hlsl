...
ConstantBuffer<Struct__GatherDOF_FloodFillFarCS_0CB> _GatherDOF_FloodFillFarCS_0CB : register(b0);
...
float2 ReadVec2STTextureRaw(in uint3 pxAndFrame, in Texture2DArray<float2> tex)
{
	...
	switch(_GatherDOF_FloodFillFarCS_0CB.LensRNGExtend)
	{
...
float2 ReadVec2STTexture(in uint3 pxAndFrame, in Texture2DArray<float2> tex)
{
	...
	if ((bool)_GatherDOF_FloodFillFarCS_0CB.JitterNoiseTextures)
...
{
	if (!(bool)_GatherDOF_FloodFillFarCS_0CB.GatherDOF_UseNoiseTextures)
	{
	...
	switch(_GatherDOF_FloodFillFarCS_0CB.LensRNGSource)
	{
...
#define FLOODFILL_TAP_COUNT _GatherDOF_FloodFillFarCS_0CB.GatherDOF_FloodFillTapCount
...
#define KernelSize _GatherDOF_FloodFillFarCS_0CB.GatherDOF_KernelSize
...
void csmain(uint3 DTid : SV_DispatchThreadID)
{
	...
	if (!(bool)_GatherDOF_FloodFillFarCS_0CB.GatherDOF_DoNearFieldFloodFill)
	{
	...
	// pass through if this is disabled
	if (!(bool)_GatherDOF_FloodFillFarCS_0CB.GatherDOF_DoFarFieldFloodFill)
	{
		BlurredFieldColorAlpha[px] = ColorCoc[px];
		return;
	}
	...
	uint noiseTextureFrameIndex = (bool)_GatherDOF_FloodFillFarCS_0CB.GatherDOF_AnimateNoiseTextures ? _GatherDOF_FloodFillFarCS_0CB.GatherDOF_FrameIndex : 0;
