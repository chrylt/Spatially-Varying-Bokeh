...
struct Struct__GatherDOF_FloodFillFarCS_1CB
{
    ...
    uint GatherDOF_DoFarFieldFloodFill;
    uint GatherDOF_DoNearFieldFloodFill;
    uint GatherDOF_FloodFillTapCount;
...
Texture2D<float4> MaxCoCTileMap : register(t0);
Texture2D<float4> ColorCoc : register(t1);
RWTexture2D<float4> BlurredFieldColorAlpha : register(u0);
...
ConstantBuffer<Struct__GatherDOF_FloodFillFarCS_1CB> _GatherDOF_FloodFillFarCS_1CB : register(b0);
...
float2 ReadVec2STTextureRaw(in uint3 pxAndFrame, in Texture2DArray<float2> tex)
{
	...
	switch(_GatherDOF_FloodFillFarCS_1CB.LensRNGExtend)
	{
...
float2 ReadVec2STTexture(in uint3 pxAndFrame, in Texture2DArray<float2> tex)
{
	...
	if ((bool)_GatherDOF_FloodFillFarCS_1CB.JitterNoiseTextures)
...
{
	if (!(bool)_GatherDOF_FloodFillFarCS_1CB.GatherDOF_UseNoiseTextures)
	{
	...
	switch(_GatherDOF_FloodFillFarCS_1CB.LensRNGSource)
	{
...
#define FLOODFILL_TAP_COUNT _GatherDOF_FloodFillFarCS_1CB.GatherDOF_FloodFillTapCount
...
#define KernelSize _GatherDOF_FloodFillFarCS_1CB.GatherDOF_KernelSize
...
void csmain(uint3 DTid : SV_DispatchThreadID)
{
	...
	if (!(bool)_GatherDOF_FloodFillFarCS_1CB.GatherDOF_DoNearFieldFloodFill)
	{
	...
	// pass through if this is disabled
	if (!(bool)_GatherDOF_FloodFillFarCS_1CB.GatherDOF_DoFarFieldFloodFill)
	{
		BlurredFieldColorAlpha[px] = ColorCoc[px];
		return;
	}
	...
	uint noiseTextureFrameIndex = (bool)_GatherDOF_FloodFillFarCS_1CB.GatherDOF_AnimateNoiseTextures ? _GatherDOF_FloodFillFarCS_1CB.GatherDOF_FrameIndex : 0;
